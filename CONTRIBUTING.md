...
├── provider-conformance/  # the shared provider test suite; resolved from source, never shipped
...
spike/                     # the throwaway de-risking spike — its own workspace, not a member
```

`spike/` is deliberately **not** a workspace member. It has its own root and lockfile, nothing in
`packages/` may depend on it, and it is kept only for the evidence it recorded: two real cloud
lifecycle transcripts and the findings memo that became the ADRs.
