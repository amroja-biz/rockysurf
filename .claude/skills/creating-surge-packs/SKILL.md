...
- **Publish it at an https URL and add it as a pack source** — the one to suggest when the user
  ...
  page, which writes the same file. **A URL ending in `.yaml` IS the pack** — no `index.json` to
  hand-write; point it at a directory instead and it is read the way the shop is, from an
  `index.json` generated by `rockysurf pack index`. The pack then shows up in the shop, Refresh
