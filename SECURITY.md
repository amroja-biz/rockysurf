...
- **A refusal writes nothing.** Every check above runs before the staged tree is renamed into
  place, so a stale digest or an `http` URL leaves `<dataDir>/providers` and the config file as
  they were.
