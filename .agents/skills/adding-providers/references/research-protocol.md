...
| 4 | `user_data` at create, cloud-init on the official Ubuntu images. **No documented ceiling** on DigitalOcean's user-data page. The value is therefore reasoned, not measured, and must be established against the API or declared with a dagger — never a round number written down because it looked plausible | `generatesUserData: true`; `userDataMaxBytes` daggered until measured |
