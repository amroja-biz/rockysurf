...
#
# `pnpm -r build` above already did this — web's build ends in `scripts/sync-web-bundle.mjs`
# since rockysurf-a29w, because leaving the copy to "the release path" meant a checkout only
# ever had a bundle if somebody ran `cp` by hand. This line is now a belt-and-braces repeat
# rather than the only copy, and it is kept so the image does not depend on that detail of
# another package's build script.
