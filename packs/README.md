...
Packs are the software bundles a Rocky Surf server can be created with — one YAML file per
pack, PR-able by anyone.
...
Five packs, ported from the pre-open-source installation scripts and rewritten against the
contract: `ai-coding-agents` (Claude Code), `amp-agents`, `codex-cli`, `gas-town`, and
`open-claw`.

`ai-coding-agents.yaml` also **defines the shared base toolchain** — the compiler, Node, the
Python bits, tmux, git and so on — that the other four reference by id. If you are adding a
