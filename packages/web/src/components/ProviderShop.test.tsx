...
/**
 * The provider half of the Rocky Surf Shop page (ADR-0028, issue #374; its own tab since #426).
 *
...
describe('the provider listing', () => {
  it('fetches nothing until the tab is the active one', async () => {
    render(
      <MemoryRouter>
        <ProviderShop active={false} isAdmin />
      </MemoryRouter>,
    )
  ...
  it('tells a member who installs providers, and asks the control plane nothing', async () => {
    render(
      <MemoryRouter>
        <ProviderShop active isAdmin={false} />
      </MemoryRouter>,
    )
