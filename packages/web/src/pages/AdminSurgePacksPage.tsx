 ...
 *
 * Ported from `frontend/src/pages/AdminSurgePacksPage.tsx`, with one change that reshapes the
...
function SurgePackFormModal({
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not save the pack')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <form onSubmit={submit} data-testid="pack-form">
      <h3>{initial ? `Edit ${initial.packId}` : 'New pack'}</h3>
...
export function AdminSurgePacksPage() {
  ...
  async function remove(pack: AdminSurgePack) {
    if (!confirm(`Delete ${pack.packId}? This removes the database row, not the YAML file.`)) return
    try {
    ...
    <main>
      ...
      <footer>
        <p style={{ fontSize: '0.875rem' }}>
          Pack images are not editable here yet — core has no upload route. Set <code>imageUrl</code> in
          the YAML file for now.
        </p>
      </footer>
    </main>
  )
