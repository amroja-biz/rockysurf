...
/**
 * The provider half of the same shop (ADR-0028).
 *
 * A provider is code that runs inside the control plane rather than a YAML file describing shell
 * that runs on a box, so the listing carries the things a person needs before consenting to
 * that: what it will ask them to configure, what its machines can and cannot do, and the one
 * sentence about what installing means. That sentence arrives on `trustSentence` — from Rocky
 * Surf, once, never as a per-entry field a registry could soften or omit.
 */
...
/** The provider's own answers to the capability questions, shown before anybody installs. */
export interface ProviderCapabilityAnswers {
  stop: boolean
...
export interface RegistryProvider {
  ...
  tarball: string
  sha256: string
  settings: ProviderSettingSummary[]
  capabilities: ProviderCapabilityAnswers
  sourceName: string
  trust: RegistryTrust
  /** The package is on disk AND the config file names it. */
  installed: boolean
  /** The version on disk, so an entry can say "update to" rather than only "install". */
...
export interface ProviderRegistryView {
  enabled: boolean
  sources: Array<{ name: string; url: string; trust: RegistryTrust }>
  shelves: ProviderShelf[]
  /** Rendered verbatim on every listing. Rocky Surf's constant, not a registry's field. */
  trustSentence: string
}

export async function getProviderRegistry(options: { refresh?: boolean } = {}): Promise<ProviderRegistryView> {
  return request<ProviderRegistryView>(`/admin/provider-registry${options.refresh ? '?refresh=1' : ''}`)
}
...
/**
 * Install — or re-install, which is how an update happens — a provider from a registry.
 *
 * Sends only the ADDRESS, never the artifact. Core refetches the listing, verifies the digest,
 * and unpacks the tarball itself; nothing that reached this page decides what lands on disk.
 */
export async function installRegistryProvider(sourceName: string, providerId: string): Promise<ProviderInstallResult> {
  return request<ProviderInstallResult>(
    `/admin/provider-registry/${encodeURIComponent(sourceName)}/${encodeURIComponent(providerId)}/install`,
    { method: 'POST' },
  )
}
...
/** Remove an installed personal provider: its package, and the config section that names it. */
export async function removePersonalProvider(providerId: string): Promise<ProviderRemovalResult> {
  return request<ProviderRemovalResult>(`/admin/personal-providers/${encodeURIComponent(providerId)}`, {
    method: 'DELETE',
  })
}
