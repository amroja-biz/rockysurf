...
describe('the rockysurf binary', () => {
  ...
  it('starts on defaults in an empty directory, with no config file at all', async () => {
    ...
    try {
      // It says there is no config file, names the path one would go at, and does not sound
      // like a failure.
      ...
      // Defaults put the data in ~/.rockysurf, not in the directory npx was run from.
