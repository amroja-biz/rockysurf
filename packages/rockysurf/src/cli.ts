  ...
  try {
    ...
    process.stderr.write(
      [
        ...
        '',
        '  Add to your MCP client, user scope (Claude Code, available in every project):',
        '',
        `    claude mcp add --scope user --env ${MCP_TOKEN_ENV}=<the token above> \\`,
        `      --env ${MCP_BASE_URL_ENV}=http://127.0.0.1:${port} -- npx -y rockysurf mcp`,
        '',
        `  For project-scope .mcp.json, Claude Desktop, or ~/.claude.json, see http://127.0.0.1:${port}/help#mcp.`,
        '',
