  ...
  try {
    ...
    process.stderr.write(
      [
        ...
        '',
        '  Add to your MCP client. Any client reads this shape:',
        '',
        '    {',
        '      "mcpServers": {',
        '        "rockysurf": {',
        ...
        '    }',
        '',
        '  Ready-made commands, user/global scope:',
        `    Claude Code: claude mcp add --scope user --env ${MCP_TOKEN_ENV}=<token> --env ${MCP_BASE_URL_ENV}=http://127.0.0.1:${port} -- npx -y rockysurf mcp`,
        `    Codex CLI:   codex mcp add rockysurf --env ${MCP_TOKEN_ENV}=<token> --env ${MCP_BASE_URL_ENV}=http://127.0.0.1:${port} -- npx -y rockysurf mcp`,
        '',
        `  Full command forms, project scope, and other clients: http://127.0.0.1:${port}/help#mcp.`,
        '',
