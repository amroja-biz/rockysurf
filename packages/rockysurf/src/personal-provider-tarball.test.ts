 ...
 *
 * **Installing a personal provider never runs a package manager.** The documented install is
 * `tar -xzf` into `<dataDir>/providers/node_modules/<name>` and a `package:` line in the config
 * file — nothing resolves a dependency for it. So a personal provider has to be SELF-CONTAINED —
 * no runtime dependencies at all — and the only way to know that stays true is to extract the
 * tarball and load it with nothing else present. A dependency added to that package's manifest in
 * six months' time fails here, in this repository, rather than in an operator's install.
 *
 * The extraction here is the install instruction from `docs/writing-a-provider.md` verbatim, so
 * what this asserts and what a human does by hand are the same steps.
 */
...
function tempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'rockysurf-provider-'))
  temporary.push(dir)
  return dir
}
