 ...
 * it under `<dataDir>/providers`, and refuses the install — naming them — if the manifest declares
 * any runtime dependency at all (issue #426). So a personal provider has to be SELF-CONTAINED — no
 * runtime dependencies at all — and the only way to know that stays true is to extract the tarball
 * and load it with nothing else present. A dependency added to that package's manifest in six
 * months' time fails here, in this repository, rather than in an operator's install.
 *
