...
/**
 * Response helpers, ported from `backend/src/lib/response.ts`.
 *
 * The error envelope is deliberately unchanged — `{ error: <human message> }` — because
 * `frontend/src/lib/api.ts` parses the body and hands it to `ApiError(status, statusText,
 * data)` untouched. Keeping the shape is what makes the SPA port mechanical. A machine-readable
