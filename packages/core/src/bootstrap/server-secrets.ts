 ...
 * deliberately small, closed, and documented in `docs/surge-pack-contract.md` and the bootstrap
...
export function createServerSecretsLoader(
  ...
  return async function loadServerSecrets(server: ServerRow): Promise<Record<string, string>> {
     ...
     * (`docs/surge-pack-contract.md`: `gh auth setup-git` works "when configured"). Shipping it only
