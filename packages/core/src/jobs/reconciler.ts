...
/**
 * A failed row can own a live instance too, and until rockysurf-4byx nothing swept it.
 *
 * `failed` is deliberately NOT in `LIVE_STATUSES`, because most failed rows never reached a
 * provider at all. But a bootstrap that fails leaves the machine UP by design — kept for
 * diagnosis and retry — and that row is not an orphan in any useful sense: core holds the
 * handle, the user can see it, and the Terminate button works. Treating it as one produced a
 * `reconciler.orphan_detected` event every five minutes for a resource nobody had lost, while
 * the thing that WAS lost — the fact that it was billing — went unrecorded.
 *
 * The handle is the whole test. With `providerData` core can describe it, cost it and kill it,
 * so it belongs on the attributable side. Without one, `provision()` threw before returning a
 * handle and any instance it left behind is genuinely unreachable from here: still an orphan,
 * still flagged, which is the only way a human hears about it.
 */
const failedRowsCoreCanStillReach = (rows: readonly ServerRow[]): ServerRow[] =>
  rows.filter((row) => row.providerData !== null)
...
export function createReconcileTick(deps: ReconcilerDeps): () => Promise<ReconcileResult> {
  ...
  return async function reconcileTick(): Promise<ReconcileResult> {
    ...
    const liveRows = [
      ...listServersByStatus(deps.db, [...LIVE_STATUSES]),
      ...failedRowsCoreCanStillReach(listServersByStatus(deps.db, ['failed'])),
    ]
