 ...
 *    installation.
 *
 * That last rule makes this a dangerous function to call with an empty `LoadResult`: it will
 * faithfully conclude that every file-backed pack has disappeared and delete the lot. Deciding
 * whether a caller has a real pack source to reconcile against is the caller's job, and for
 * the boot path `syncPacksAtBoot` does it — see rockysurf-96ce for what happened when it did
 * not. This function stays a plain reconcile: given a set, make the database match it.
 */
