...
export type FakeMethod = 'validateCredentials' | 'validateSpec' | 'provision' | 'describe' | 'terminate' | 'stop' | 'start'
...
export function makeFakeProvider(options: FakeProviderOptions = {}): FakeProvider {
  ...
  const capabilities: ProviderCapabilities = { ...DEFAULT_CAPABILITIES, ...options.capabilities }
