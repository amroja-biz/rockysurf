 ...
 *     this package, so a declared runtime dependency would be an import failure at the next
 *     restart — reported as a provider that will not load, hours after the operator thought they
 *     had installed it. A package that declares ANY runtime dependency is refused (issue #426),
...
/**
 * Every runtime dependency the manifest declares. ANY is a refusal (issue #426, after #417's
 * ruling that a personal provider takes no runtime dependencies): nothing here resolves one, an
 * import that fails at the next restart would be reported hours later as "the provider did not
 * load", and a package that happens to find a copy under `<dataDir>/providers` today is one that
 * breaks when that copy is removed by hand. The refusal names them so the author knows what to
 * bundle.
 */
function declaredDependencies(manifest: PersonalProviderManifest): string[] {
  ...
  try {
    ...
    if (dependencies.length > 0) {
      return {
        ok: false,
        reason:
          `${entry.package} declares runtime dependencies: ${dependencies.join(', ')}. ` +
          'Rocky Surf never runs npm, so a provider published to the shop has to carry everything it ' +
          'needs with no dependencies at all — see docs/writing-a-provider.md, "Publishing to the shop".',
      }
    }
