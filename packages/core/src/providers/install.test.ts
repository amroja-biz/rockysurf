...
describe('installing a provider package', () => {
  ...
  it('refuses a package with runtime dependencies, and names them', async () => {
    const bytes = packageOf(
  ...
  it('refuses a declared dependency even when a copy is already on disk (issue #426)', async () => {
    const bytes = packageOf(
      providerPackageMembers({
        name: '@fixture/rockysurf-provider-nimbus',
        version: '1.0.0',
    ...
    expect(result).toMatchObject({ ok: false })
    if (result.ok) return
    expect(result.reason).toContain('declares runtime dependencies: zod')
    expect(existsSync(providerPackageDir(providersDir, '@fixture/rockysurf-provider-nimbus'))).toBe(false)
  })
