...
// NOTE: `packages/provider-aws/src/feed.test.ts` is this file's twin, same as the module it
...
const DOC = {
  ...
  currency: 'USD',
  regions: { eastus: { 'Standard_B2ps_v2': 0.0816, 'Standard_B2s_v2': 0.0168 } },
}
...
describe('parsePriceFeedDoc', () => {
  ...
  it('rejects the WHOLE document on one bad price, never serving the rest', () => {
    ...
    for (const bad of [0, -1, Number.NaN, Number.POSITIVE_INFINITY, '0.08', null]) {
      const doc = { ...DOC, regions: { eastus: { 'Standard_B2ps_v2': 0.0816, evil: bad } } }
...
describe('PriceFeedClient', () => {
  ...
  it('shares one in-flight fetch between concurrent callers', async () => {
    const fetchSpy = vi.fn(async () => jsonResponse(DOC))
    vi.stubGlobal('fetch', fetchSpy)
    const client = new PriceFeedClient('https://feed.test/azure.json', 6, () => 0)
  ...
  it('degrades to null on a malformed body and on a network error', async () => {
    ...
    expect(await new PriceFeedClient('https://feed.test/azure.json', 6, () => 0).get()).toBeNull()
