...
export interface InstanceView {
  ...
  sshPort?: number
  /**
   * Where a human can open THIS instance in the provider's own management console
   ...
   *
   * The PROVIDER reports it because the URL shape is provider knowledge, and core holds none —
   * `provider.id` conditionals in shared code are grep-enforced out. It rides on the view
   * rather than on a static method on the provider because the answer is per-instance: every
   * console URL contains the instance's own id, which only the handle knows.
   *
   * It carries no secret. An instance id and a region are identifiers, not credentials, and
   * the link lands on the provider's own sign-in page for anyone not already logged in.
   */
  consoleUrl?: string
  /**
