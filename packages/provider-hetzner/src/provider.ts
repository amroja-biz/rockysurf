...
export function makeHetznerProvider(input: HetznerProviderInput): ComputeProvider {
  const { location, image, managedBy } = input
   ...
   * SECOND kind of cloud resource. A key matched by fingerprint is NOT claimed as owned: it
   * already existed, it may be shared with other servers or belong to someone else entirely,
   * and reaping it on terminate would break them (ADR-0003, D2).
   */
  async function ensureSshKey(
    ...
    if (existing.ssh_keys[0]) return { id: existing.ssh_keys[0].id, created: false }
    ...
    try {
      ...
      return { id: result.ssh_key.id, created: true }
    } catch (err) {
      if (hetznerCodeOf(err) !== 'uniqueness_error') throw err
      // Raced with a concurrent provision, or a leftover holds the name.
      const retry = await api.call<{ ssh_keys: HetznerSshKey[] }>('GET', query)
      if (retry.ssh_keys[0]) return { id: retry.ssh_keys[0].id, created: false }
      const byName = await api.call<{ ssh_keys: HetznerSshKey[] }>(
        'GET',
        `/ssh_keys?name=${encodeURIComponent(name)}`,
      )
      if (byName.ssh_keys[0]) return { id: byName.ssh_keys[0].id, created: false }
      throw err
    }
  }
  ...
  return {
    ...
    async provision(spec: ProvisionSpec): Promise<ProvisionResult> {
      ...
      for (let i = 0; i < spec.sshPublicKeys.length; i++) {
        const { id, created } = await ensureSshKey(spec.sshPublicKeys[i]!, `${name}-key-${i}`, labels)
        sshKeyIds.push(id)
        if (created) ownedSshKeyIds.push(id)
      }
