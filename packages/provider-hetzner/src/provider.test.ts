...
import hetznerProviderFactory, { asHetznerData, makeHetznerProvider, sshFingerprint } from './index.js'
...
describe('provision', () => {
  ...
  it('recovers from a raced key creation', async () => {
    ...
    const { data } = await build().provision(SPEC)
    expect(data['ownedSshKeyIds']).toEqual([]) // resolved, not created → not owned
