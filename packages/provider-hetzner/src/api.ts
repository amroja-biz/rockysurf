 ...
 * this provider: the entire transport is this one file, against a documented REST API, with no
 * transitive dependency tree to audit or to slow an `npx` cold start.
 *
 * NO LINE COUNT LIVES HERE ANY MORE. This comment used to claim "~600 lines" for the package,
 * which was never true — the real figure was 1,036 — and it had propagated into two other
 * documents before anyone counted (`rockysurf-z3uz`). A measured number written inside the
 * thing it measures goes stale on the next edit, including the edit that corrects it. The
 * counts now live, dated, in `docs/writing-a-provider.md` under "Vendor SDKs", which is where
 * the argument they support is made.
 */
