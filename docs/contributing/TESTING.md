...
path. CI runs it as the `Docker install (compose)` job on every pull request and every push to
`main`, except for changes that touch only docs.
...
itself and sets two outputs. A pull request confined to `packages/web/`, `docs/`, `.claude/`,
`.agents/skills/`, `LICENSE` or Markdown runs `Typecheck`, `Test`, `Secret scan`,
`UI (browser)` and, unless it touches only docs, `Docker install (compose)`. Anything beyond that
also runs `Lint (structure)`, `Release tarballs` and `Push bootstrap (real sshd)`. Pushes to
`main` run every job, except that a docs-only push skips `Docker install (compose)`. For a push,
the file list comes from comparing the push's before and after commits. `Pack smoke` is its own workflow
