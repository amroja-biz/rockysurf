...
`docs/surge-pack-contract.md` promises exactly two names — `$GITHUB_TOKEN` and `$RDP_PASSWORD` — and
...
boxes built from that pack. `surge-pack-contract.md`'s "closed list" paragraph is amended to say
...
- `docs/surge-pack-contract.md` § `inputs`; `docs/bootstrap-contract.md` § The `secrets.env` key-name
