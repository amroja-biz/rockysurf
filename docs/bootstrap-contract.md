...
|---|---|---|
| `GITHUB_TOKEN` | the **user** — one token, reused across their servers | `secretsStore.getGithubToken(userId)` |
| `RDP_PASSWORD` | the **server** — it is set on that box's own `rocky` account | `secretsStore.getRdpPassword(serverId)` |
