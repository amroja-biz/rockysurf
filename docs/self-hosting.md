...
registry:
  enabled: true
  # baseUrl: https://raw.githubusercontent.com/amroja-biz/rockysurf-shop/main
  cacheTtlSeconds: 300
```

**Nothing here is read at boot.** The registry is fetched when you open the shop, never during
...
which is deliberately a different message from a registry it could not reach.

Fetches are plain file GETs against `baseUrl`: `<baseUrl>/index.json` for the listing, then the
paths that listing names. **Not the GitHub API**, whose unauthenticated quota is 60 requests per
hour shared across everything on one source IP — a control plane behind a corporate NAT would
spend that before it had listed the shop once. `baseUrl` goes through the same SSRF guard as pack
import (see [`SECURITY.md`](../SECURITY.md) § Server-side fetch policy), so a URL resolving to a
private address is refused whatever you set.

Point `baseUrl` at your own repository to run a fork or an internal shop. The layout it expects
is the shop's: `index.json` at the root, generated by `rockysurf pack index`.

### What "official" means, and what the checks prove

The index labels every pack `official` or `community`, and the UI shows the label wherever a pack
appears.

- **`official`** — mirrored into the shop by the main repository's CI from its own `packs/`. It is
  a statement about *provenance*, not a quality score: a contributor cannot set it on their own
  pack, because they cannot write the directory it is mirrored into.
- **`community`** — contributed by pull request, having passed `rockysurf pack lint` and
  `rockysurf pack check` and been read by a maintainer.

Those checks prove a pack is **well-formed and survives being resumed**. They do not prove it is
safe, and nothing in Rocky Surf claims they do: an install script is arbitrary shell that runs as
**root** on your box, and no schema check can decide whether it is benign. Before installing
anything from the registry the admin UI shows you every script it will run, verbatim, along with
which steps run as root and every URL they fetch. That disclosure is the control. Read it.
...
halves — so the trust chain is the shop repository's `main` branch and GitHub's account controls,
and no more than that.

A pack installed from the registry lands as a **database row with no `sourceFile`**, exactly like
