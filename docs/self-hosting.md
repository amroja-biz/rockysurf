...
There are two ways to install one: **from the Rocky Surf Shop tab** in the app, which does
...
Shop](https://github.com/amroja-biz/rockysurf-shop#providers).
...
The Surge Packs page shows bundled and registry packs together, each labelled with where it came
from; the Rocky Surf Shop tab shows what each registry lists, installed or not.
...
A refusal at any step writes nothing: a listing with a stale `sha256`, or one whose tarball is an
...
It lives on the **Rocky Surf Shop** tab (`/shop`) and on the Community sub-tab of **Surge Packs**
(`/packs`), which is also where you can see what you already have and
