...
[`CONTRIBUTING.md`](../../CONTRIBUTING.md#a-note-on-this-repositorys-history)).
...
| [`spike/`](spike/) | The de-risking spike's findings memo, interface sketch and real-cloud recordings — the evidence behind ADR-0001 to ADR-0004 and [`docs/bootstrap-contract.md`](../bootstrap-contract.md), which key their amendment ids and finding numbers to it. |

The two phase specifications that used to sit here (`SPEC-P1.md`, the dogfooding build, and
...
exists. The spike memo is the input the ADRs were argued from, not a description of what shipped:
where the two disagree, the ADR wins. For how the current system works, start at
[`docs/adr/llms.txt`](../adr/llms.txt); for running it,
[`docs/self-hosting.md`](../self-hosting.md).

The log is kept because the reasoning is reusable and the mistakes were expensive. The AWS lessons
in it were generalized into the AWS learnings library at
...
`DEVLOG.md` was scrubbed before being moved here: an AWS account id, an API Gateway id and a
